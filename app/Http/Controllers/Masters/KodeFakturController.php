...
class KodeFakturController extends Controller
{
    ...
    public function index()
    {
        ...
        $category_invoice = DB::select("EXEC SP_GetMasterJenisFaktur");
    ...
    public function store(request $request)
    {
        try {
            if(empty($request->invoice_id))
            {
                ...
                $store = DB::statement('EXECUTE SP_MasterFaktur :faktur_id, :project_id, :company_id, :user_id, :faktur_code, :faktur_name, :jenis_faktur_id, :ppn, :account_number, :account_div, :created_by, :created_date, :updated_by, :updated_date, :ip_address, :mac_address, :State', [
                    'faktur_id' => 0,
                    'project_id' => session("project"),
                    'company_id' => session("company"),
                    'user_id' => Auth::user()->user_id,
                    'faktur_code' => $request->invoice_code,
                    'faktur_name' => $request->invoice_name,
                    'jenis_faktur_id' => $request->category_invoice,
                    'ppn' => $request->ppn,
                ...
                $master_faktur = DB::select("EXEC SP_GetMasterFakturById :faktur_id", [
                    'faktur_id' => $request->invoice_id
                ]);
                // update master faktur
                $update = DB::statement('EXECUTE SP_MasterFaktur :faktur_id, :project_id, :company_id, :user_id, :faktur_code, :faktur_name, :jenis_faktur_id, :ppn, :account_number, :account_div, :created_by, :created_date, :updated_by, :updated_date, :ip_address, :mac_address, :State', [
                    'faktur_id' => $request->invoice_id,
                    'project_id' => session("project"),
                    'company_id' => session("company"),
                    'user_id' => Auth::user()->user_id,
